...
impl Solver {
    ...
    fn solve(&mut self) {
        ...
        let mut ans = mod_pow(2, n) - 1;
        let mut v = vec![1_usize; b];
        v[0] = n;
        for k in 1..b {
            v[k] = v[k - 1] * (n - k);
            v[k] %= MOD;
        }
        let mut count_a = v[a - 1];
        for k in 1..=a {
            count_a = mod_inv(count_a, k)
        }
        let mut count_b = v[b - 1];
        for k in 1..=b {
            count_b = mod_inv(count_b, k)
        }
        ans = (MOD + ans - count_a) % MOD;
        ans = (MOD + ans - count_b) % MOD;
        println!("{}", ans);
    }
}
