...
impl Solver {
    ...
    fn solve(&mut self) {
        ...
        for i in 1..=N {
            if color[i] == 0 {
                let (black_num, white_num) = dfs(i, &G, &mut color, 1);
                if black_num == -1 {
                    println!("0");
                    return;
                }
...
fn dfs(pos: usize, G: &Vec<Vec<usize>>, color: &mut Vec<isize>, c: isize) -> (isize, isize) {
    ...
    for &next in &G[pos] {
        ...
        let (black_num, white_num) = dfs(next, G, color, color[next]);
        if black_num == -1 {
            return (-1, -1);
        }
        ret.0 += black_num;
