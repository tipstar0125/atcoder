    ...
    fn solve(&mut self) {
        ...
        for _ in 0..Q {
            ...
            if t == 1 {
                ...
                let pos = r * W + c;
                if r > 0 && matrix[r - 1][c] {
                    let neighbor = (r - 1) * W + c;
                    uf.unite(pos, neighbor);
                }
                if r < H - 1 && matrix[r + 1][c] {
                    let neighbor = (r + 1) * W + c;
                    uf.unite(pos, neighbor);
                }
                if c > 0 && matrix[r][c - 1] {
                    let neighbor = r * W + c - 1;
                    uf.unite(pos, neighbor);
                }
                if c < W - 1 && matrix[r][c + 1] {
                    let neighbor = r * W + c + 1;
                    uf.unite(pos, neighbor);
                }
            } else {
                ...
                let start = ra * W + ca;
                let goal = rb * W + cb;
                if matrix[ra][ca] && matrix[rb][cb] && uf.is_same(start, goal) {
                    println!("Yes");
                } else {
                    println!("No");
                }
