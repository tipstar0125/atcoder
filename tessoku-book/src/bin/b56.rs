...
impl Solver {
    ...
    fn solve(&mut self) {
        ...
        let mut H2 = vec![h1];
        for i in 0..N {
            let num1 = (S[i] as u8 - b'a' + 1) as usize;
            let num2 = (S[N - i - 1] as u8 - b'a' + 1) as usize;
            h1 *= B;
            h2 *= B;
            h1 += num1;
            h2 += num2;
        ...
        let hash_calc1 = |l: usize, r: usize| -> usize {
            let mut ret = MOD + H1[r] - H1[l - 1] * power100[r - l + 1] % MOD;
            ret %= MOD;
            ret
        };
        let hash_calc2 = |l: usize, r: usize| -> usize {
            let l1 = N - r + 1;
            let r1 = l1 + r - l;
            let mut ret = MOD + H2[r1] - H2[l1 - 1] * power100[r1 - l1 + 1] % MOD;
            ret %= MOD;
            ret
        };

        for &(l, r) in &LR {
            if hash_calc1(l, r) == hash_calc2(l, r) {
                println!("Yes");
            } else {
                println!("No");
            }
