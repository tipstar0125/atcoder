...
impl Solver {
    ...
    fn solve(&mut self) {
        ...
        let mut color: Vec<BTreeMap<usize, usize>> = vec![BTreeMap::new(); N];
        ...
        while !Q.is_empty() {
            ...
            for &(next, num) in &G[pos] {
                if !visited[next] {
                    let mut c = 0;
                    if color[pos].is_empty() {
                        color[pos].insert(0, 0);
                    } else {
                        let (&k, &v) = color[pos].iter().next().unwrap();
                        if k == 0 {
                            c = v + 1;
                            color[pos].insert(0, c);
                        } else {
                            color[pos].insert(0, 0);
                        }
                    }
                    ...
                    if color[pos].len() >= 2 {
                        let mut iter = color[pos].iter();
                        let (&k0, &v0) = iter.next().unwrap();
                        let (&k1, &v1) = iter.next().unwrap();
                        if v0 + 1 == k1 {
                            color[pos].insert(k0, v1);
                            color[pos].remove(&k1);
                        }
                    }

                    color[next].insert(c, c);
                    ans[num] = c + 1;
