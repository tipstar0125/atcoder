...
impl Solver {
    ...
    fn solve(&mut self) {
        ...
        X.sort();
        Y.sort();
        let x_vec = vec![X[0], X[1], X[N - 2], X[N - 1]];
        let y_vec = vec![Y[0], Y[1], Y[N - 2], Y[N - 1]];
        let mut v = vec![];
        for i in 0..2 {
            for j in 2..4 {
                v.push(x_vec[j] - x_vec[i]);
                v.push(y_vec[j] - y_vec[i]);
            }
        }
        v.sort();
        v.reverse();
        if (XY[0].0 - XY[N - 1].0).abs() == (XY[0].1 - XY[N - 1].1).abs() {
            println!("{}", v[2]);
        } else {
            println!("{}", v[1]);
        }
