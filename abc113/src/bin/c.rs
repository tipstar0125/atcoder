...
impl Solver {
    ...
    fn solve(&mut self) {
        ...
        let mut ans = vec!["".to_string(); M + 1];
        for (i, py) in PY_list[1..].iter().enumerate() {
            let num_zero = 6 - (i + 1).to_string().len();
            let mut number = "".to_string();
            for _ in 0..num_zero {
                number += "0".to_string().as_str();
            }
            number += (i + 1).to_string().as_str();

            let L = py.len();
            for j in 1..L + 1 {
                let (_, index) = py[j - 1];
                let num_zero2 = 6 - j.to_string().len();
                let mut number2 = number.clone();
                for _ in 0..num_zero2 {
                    number2 += "0".to_string().as_str();
                }
                number2 += j.to_string().as_str();
                ans[index + 1] = number2;
            }
        }
        for i in 1..=M {
