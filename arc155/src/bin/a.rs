...
impl Solver {
    ...
    fn solve(&mut self) {
        ...
        for _ in 0..T {
            ...
            let S_rev: String = S.chars().rev().collect::<String>();
            if S == S_rev {
                println!("Yes");
                continue;
            }
            if N % 2 == 0 || N % 3 == 0 {
                if N % 2 == 0 {
                    let s1 = &S[..N / 2];
                    let s2 = &S[N / 2..];
                    let set1: BTreeSet<char> = s1.chars().into_iter().collect();
                    let set2: BTreeSet<char> = s2.chars().into_iter().collect();
                ...
                if N % 3 == 0 {
                    let s1 = &S[..N / 3];
                    let s2 = &S_rev[N / 3..N - N / 3];
                    let s3 = &S[N - N / 3..];
                    if K >= N / 3 && K % (N / 3) == 0 && s1 == s2 && s1 == s3 {
                        println!("Yes");
                        continue;
                    }
                }
