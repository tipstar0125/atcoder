...
impl Solver {
    ...
    fn solve(&mut self) {
        ...
        for _ in 0..T {
            ...
            let mut ans = 10_usize.pow(N.to_string().len() as u32 - 1) - 1;
            ...
            for i in 1..=9 {
                ...
                if L % i == 0 && L / i > 1 {
                    let s1 = &s[..i];
                    let s2 = (s1.parse::<usize>().unwrap() - 1).to_string();
                    for _ in 0..L / i {
                        v1.push(s1);
                        v2.push(&s2);
                    }
                    let num1: usize = v1.iter().join("").parse().unwrap();
                    let num2: usize = v2.iter().join("").parse().unwrap();
                    if num1 <= N {
                        ans = max!(ans, num1);
                    }
                    if num2 <= N {
