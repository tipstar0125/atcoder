...
impl Solver {
    ...
    fn solve(&mut self) {
        ...
        let mut mp_l: BTreeMap<usize, Vec<usize>> = BTreeMap::new();
        let mut destroyed = vec![false; N];
        for (i, &(l, r)) in LR.iter().enumerate() {
            heap.push((Reverse(r), i));
            mp_l.entry(l).or_default().push(i);
        }

        let mut ans = 0_usize;
        let mut min = 1_usize;
        while !heap.is_empty() {
            let (Reverse(r), idx) = heap.pop().unwrap();
            if !destroyed[idx] {
                destroyed[idx] = true;
                ans += 1;
                let mut optional_range = mp_l.range(min..=r + D - 1);
                loop {
                    let next = optional_range.next();
                    if next.is_none() {
                        break;
                    }
                    let destroy_list = next.unwrap().1;
