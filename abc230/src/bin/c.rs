...
impl Solver {
    ...
    fn solve(&mut self) {
        ...
        let mut ans = vec![vec!['.'; S as usize - R as usize + 1]; Q as usize - P as usize + 1];

        for (i, ii) in (P..=Q).into_iter().enumerate() {
            for (j, jj) in (R..=S).into_iter().enumerate() {
                let k1 = ii - A;
                let k2 = jj - B;
                if k1 == k2
                    && max!(1 - A, 1 - B) <= k1
                    && k1 <= min!(N as isize - A, N as isize - B)
                {
                    ans[i][j] = '#';
                }
                let k1 = ii - A;
                let k2 = -(jj - B);
                if k1 == k2
                    && max!(1 - A, B - N as isize) <= k1
                    && k1 <= min!(N as isize - A, B - 1)
                {
                    ans[i][j] = '#';
                }
            }
