...
impl Solver {
    ...
    fn solve(&mut self) {
        ...
        let dr: Vec<isize> = vec![1, -1, 0, 0];
        let dc: Vec<isize> = vec![0, 0, 1, -1];

        while !Q.is_empty() {
            let (row, col) = Q.pop_front().unwrap();
            for (r, c) in dr.iter().zip(dc.iter()) {
                ...
                if nr >= 0
                    ...
                    && nr < H as isize
                    && nc < W as isize
                    && d[row][col] + 1 < d[nr as usize][nc as usize]
                {
                    d[nr as usize][nc as usize] = d[row][col] + 1;
