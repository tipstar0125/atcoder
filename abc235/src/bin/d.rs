...
impl Solver {
    ...
    fn solve(&mut self) {
        ...
        let upper_limit = 10_usize.pow(N.to_string().len() as u32);
        let mut Q = VecDeque::new();
        let mut visited = vec![-1; upper_limit];
        Q.push_back(1);
        visited[1] = 0;
        ...
        while !Q.is_empty() {
            ...
            let vv = v * a;
            if vv < upper_limit && visited[vv] == -1 {
                visited[vv] = visited[v] + 1;
                Q.push_back(vv);
            }

            let mut num_str = v.to_string();
            let length = num_str.len();
            if v >= 10 && v % 10 != 0 {
                let str1 = &num_str[length - 1..];
                let str2 = &num_str[..length - 1];
                num_str = "".to_string() + str1 + str2;
