...
impl Solver {
    ...
    fn solve(&mut self) {
        ...
        for i in 1..=N {
            ...
            let head_num = (S[0] as u8 - b'0') as usize;
            let tail_num = (S[S.len() - 1] as u8 - b'0') as usize;
            *cnt.entry((head_num, tail_num)).or_default() += 1;
        }
        let mut ans = 0_usize;
        for i in 1..=9 {
            for j in 1..=9 {
                let cnt1 = *cnt.entry((i, j)).or_default();
                let cnt2 = *cnt.entry((j, i)).or_default();
                ans += cnt1 * cnt2;
            }
        }
        println!("{}", ans);
