...
impl Solver {
    ...
    fn solve(&mut self) {
        ...
        for _ in 0..T {
            ...
            let keys: Vec<usize> = pf.keys().cloned().collect();
            if pf[&keys[0]] == 1 && pf[&keys[1]] == 1 {
                let v1 = keys[0];
                let v2 = keys[1];
                let vv1 = (v1 as f64).sqrt() as usize;
                let vv2 = (v2 as f64).sqrt() as usize;
                if vv1 * vv1 == v1 {
                    let p = vv1;
                    let q = v2;
                    println!("{} {}", p, q);
                ...
                let p = if pf[&keys[0]] == 2 { keys[0] } else { keys[1] };
                let q = if pf[&keys[0]] == 1 { keys[0] } else { keys[1] };
                println!("{} {}", p, q);
            }
        }
    }
