...
impl Solver {
    ...
    fn solve(&mut self) {
        ...
        for &(start, stop) in &UV {
            let mut point = vec![0_usize; N];
            let mut dist = vec![INF; N];
            let mut Q = VecDeque::new();
            point[start] = A[start];
            dist[start] = 0;
            Q.push_back((start, point[start]));

            while !Q.is_empty() {
                let (pos, p) = Q.pop_front().unwrap();
                if p != point[pos] {
                    continue;
                }
                for &next in &G[pos] {
                    if dist[next] >= dist[pos] + 1 && point[next] < point[pos] + A[next] {
                        point[next] = point[pos] + A[next];
                        dist[next] = dist[pos] + 1;
                        Q.push_back((next, point[pos] + A[next]));
                    }
                }
            }
            if dist[stop] == INF {
