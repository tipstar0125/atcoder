...
impl Solver {
    ...
    fn solve(&mut self) {
        ...
        let N = S.len();
        let mut L_position = vec![0; N];
        L_position[N - 1] = N - 1;
        for i in (0..N - 1).rev() {
            if S[i] == 'L' {
                L_position[i] = i;
            } else {
                L_position[i] = L_position[i + 1];
            }
        }

        let mut R_position = vec![0; N];
        R_position[0] = 0;
        for i in 1..N {
            if S[i] == 'R' {
                R_position[i] = i;
            } else {
                R_position[i] = R_position[i - 1];
            }
        }
