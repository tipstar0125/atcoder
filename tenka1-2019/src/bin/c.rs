...
impl Solver {
    ...
    fn solve(&mut self) {
        ...
        let mut black_S = vec![0_usize; N + 1];
        for i in 1..=N {
            if S[i - 1] == '#' {
                black_S[i] = black_S[i - 1] + 1;
            } else {
                black_S[i] = black_S[i - 1];
            }
        }

        let mut ans = 1_usize << 60;
        let mut white_cnt = 0_usize;
        let mut black_cnt = 0_usize;
        for i in (0..N).rev() {
            if S[i] == '.' {
                white_cnt += 1;
            } else {
            ...
            ans = min!(ans, white_cnt + black_S[i]);
        }
