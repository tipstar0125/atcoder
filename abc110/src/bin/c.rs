...
impl Solver {
    ...
    fn solve(&mut self) {
        ...
        let N = S.len();
        let mut map1: BTreeMap<char, BTreeSet<char>> = BTreeMap::new();
        let mut map2: BTreeMap<char, BTreeSet<char>> = BTreeMap::new();
        for i in 0..N {
            map1.entry(T[i]).or_default().insert(S[i]);
            map2.entry(S[i]).or_default().insert(T[i]);
        }
        let mut ok = true;
        for (_, v) in map1 {
            if v.len() > 1 {
