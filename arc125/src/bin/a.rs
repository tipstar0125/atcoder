...
impl Solver {
    ...
    fn solve(&mut self) {
        ...
        let mut s_vec = vec![vec![]; 2];
        let mut t_vec = vec![vec![]; 2];
        for i in 0..N {
            s_vec[S[i]].push(i);
        }
        for i in 0..M {
        ...
        if !t_vec[0].is_empty() && s_vec[0].is_empty() {
            println!("-1");
            return;
        }
        if !t_vec[1].is_empty() && s_vec[1].is_empty() {
            println!("-1");
            return;
        }
        if s_vec[0].is_empty() {
        ...
        let mut num = S[0];
        let mut ans = if S[0] == 0 {
            min!(s_vec[1][0], N - s_vec[1][s_vec[1].len() - 1]) - 1
        } else {
            min!(s_vec[0][0], N - s_vec[0][s_vec[0].len() - 1]) - 1
        };
        if t_vec[S[0] ^ 1].is_empty() {
            ans = 0;
        }
        for i in 0..M {
            if T[i] == num {
